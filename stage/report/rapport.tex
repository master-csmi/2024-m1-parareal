...
\begin{enumerate}
    \item \textbf{Time Decomposition:} 
    
        Split the total time interval \([0, T]\) into \(N\) sub-intervals: \([T_0, T_1], [T_1, T_2], ..., [T_{N-1}, T_N]\). Each sub-interval can be processed independently once the initial conditions are set.
    \item \textbf{Coarse Propagation:} 
    ...
    \item \textbf{Parallel Fine Propagation:} 
...
\WHILE{$|U^k-U^{k-1}| > \epsilon $}
    \begin{itemize}
        ...
        \item communicate fine solution and update u in sequential and share it to all process: $\;\;\;\;\;\;\;\;\; j = 0,1, ... ,N-1$.
        \newline
...
\subsubsection{Overview of the Python Package}
...
\begin{itemize}
    \item \textbf{Core Features:} 
        \begin{itemize}
            \item \textbf{Solver Integration:} The package supports multiple ODE solvers, including explicit and implicit Euler methods, and Runge-Kutta methods of different orders as well as scipy ode solvers. These solvers can be used interchangeably within the Parareal framework.
            \item \textbf{Parallelization Support:} The package leverages Python's multiprocessing library \textbf{MPI} to parallelize computations across multiple cores. This enables efficient execution of the Parareal algorithm on multi-core systems.
            \item \textbf{Customization:} Users can define their own coarse and fine solvers, adjust time intervals, and set convergence tolerances and max number of parareal iteration, making the package highly adaptable to different problem domains.
        \end{itemize}
\end{itemize}
...
To assess the number of iterations required for convergence and to determine the convergence order, the different systems (System 1, System 2, System 3) were tested. These tests were conducted using all possible combinations of coarse and fine solvers.
...
\begin{enumerate}
    ...
    \item \textbf{Convergence Order:}
        \begin{itemize}
            \item \textbf{Convergence Order Calculation:}
            the convergence order was computed by the following formula:
            \begin{equation}
                \large
            ...
            were:
            \begin{itemize}
                \item $e_i$  is the error after the $i$ iteration
                \item $\Delta_i$ is the time step used in the $i$ iteration.
            \end{itemize}

        \end{itemize}
        For each combination of coarse and fine solvers, the convergence order $P$ was calculated by running the Parareal algorithm on all tests systems. The error
        was measured as The difference between the numerical and exact solutions obtained analytically.
\end{enumerate}

\subsubsection{Execution Time and Speedup Tests}
To evaluate the execution time and speedup, the Lorenz system was used for all possible combinations of coarse and fine solvers available.
The steps for these tests were:
\begin{enumerate}
    \item \textbf{Initial Setup:}
        \begin{itemize}
    ...
    \item \textbf{Parallel Execution:}
        \begin{itemize}
            \item The Lorenz system was solved using different numbers of $P$ processes (up to 128) with MPI for parallel execution.
...
\begin{itemize}
    \item \textbf{NumPy:} For efficient numerical operations and array manipulations.
    \item \textbf{mpi4Py:} For parallel execution using MPI (Message Passing Interface), allowing the Parareal algorithm to be executed across multiple processes.
    \item \textbf{Matplotlib:} For generating plots and visualizations of the results.
    \item \textbf{Seaborn:} for creating informative and attractive statistical graphics.
...
\begin{itemize}
    \item \textbf{Test Configuration:} Define parameters specific to the test, such as test file names, parameters, and configurations.
    \item \textbf{Test Execution}: Commands to execute the test, calling a Python script with appropriate arguments.
    \item \textbf{Result Handling:} Save results, and clean up temporary files.
\end{itemize}

\subsection{Code Structure}
The project is available on  \href{https://github.com/master-csmi/2024-m1-parareal}{github} and is structured as shown in the picture below \reff{fig:8}:


\begin{figure}[ht!]
    \centering
    \includegraphics[width=.4\textwidth]{img/code_struc.jpg}
    \caption{Code structure}
    \label{fig:8}
\end{figure}

\begin{itemize}
    \item \textbf{convergence\_order\_test.sh:} This shell script automates the execution of the convergence order test using the Python script convergence\_order\_test.py. To run the script, use the following command in the terminal:
    \newline
    \begin{lstlisting}[language=bash]
    ./convergence_order_test.sh <nbr_of_mpi_processes>
    \end{lstlisting}
    Replace <nbr\_of\_mpi\_processes> with the desired number of MPI processes.
    \item \textbf{nbr\_iter\_test.sh:} This shell script automates the execution of the number of parareal itertation test using the Python script nbr\_iter\_test.py. To run the script, use the following command in the terminal:
    \begin{lstlisting}[language=bash]
    ./nbr_iter_test.sh.sh <nbr_of_mpi_processes>
    \end{lstlisting}
    Replace <nbr\_of\_mpi\_processes> with the desired number of MPI processes.
    \item \textbf{speedup\_testt.sh:} This shell script automates the execution of the number of parareal speedup test using the Python script speedup\_test.py. To run the script, use the following command in the terminal:
    \begin{lstlisting}[language=bash]
    ./speedup_test.sh
    \end{lstlisting}
\end{itemize}
