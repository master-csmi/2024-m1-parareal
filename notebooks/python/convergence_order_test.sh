...
job_ids_file="${output_dir}/job_ids_cv_order_${timestamp}.txt"
...
#SBATCH --output=run-parareal-cv_order-${func}.out
#SBATCH --error=run-parareal-cv_order-${func}.err

export OMP_NUM_THREADS=1
mpiexec -bind-to core python convergence_order_test.py $func 
EOF
...
data_file="${output_dir}/data_cv_order_${timestamp}.txt"
for func in "${functions[@]}"
do
    result_file="data-cv_order-${func}.txt"
